...
// Modificamos para aceptar la URL completa en lugar de solo el nombre
...
// Modificamos para aceptar la URL completa
exports.actualizarProducto = async (idmarca, idtipo, imagen, detalle, cilindrada, precio, titulo, id) => {
    const {rows} = await pool.query(
        "UPDATE producto SET idmarca=$1, idtipo=$2, imagen=$3, detalle=$4, cilindrada=$5, precio=$6, titulo=$7 WHERE id=$8 RETURNING *",
        [idmarca, idtipo, imagen, detalle, cilindrada, precio, titulo, id]
    );
    return rows[0];
}
